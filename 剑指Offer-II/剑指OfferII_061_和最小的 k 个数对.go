...
func (h hp061) Len() int {
	return len(h.data)
}

func (h hp061) Less(i, j int) bool {
	a, b := h.data[i], h.data[j]
	return h.nums1[a.i]+h.nums2[a.j] < h.nums1[b.i]+h.nums2[b.j]
}

func (h hp061) Swap(i, j int) {
	h.data[i], h.data[j] = h.data[j], h.data[i]
}

func (h *hp061) Push(v interface{}) {
	h.data = append(h.data, v.(struct{ i, j int }))
}

func (h *hp061) Pop() interface{} {
	tmp := h.data
	v := tmp[len(tmp)-1]
